...
def create_enhanced_plots_with_save(results_16x, results_24x):
    """Create improved 9-panel dashboard with all requested enhancements"""
    
    # Create figure with increased size for better spacing
    fig = plt.figure(figsize=(20, 16))
    
    # IMPROVEMENT #2: Increase subplot spacing significantly
    plt.subplots_adjust(
        left=0.06,
        bottom=0.06,
        right=0.96,
        top=0.94,
        wspace=0.35,    # Increased width spacing between subplots
        hspace=0.45     # Increased height spacing between subplots
    )
    ...
    # 4. Final Equity Distribution - IMPROVEMENT #1: X-axis limited to 1500
    ax4 = plt.subplot(3, 3, 4)
    if results_16x['final_equities']:
        # Filter data for better visualization (robust approach)
    ...
    plt.xlim(0, 1500)  # KEY IMPROVEMENT: X-axis limit set to 1500
    ...
    plt.ylim(-100, 1400)  # Limit Y-axis for better visibility
    ...
    # IMPROVEMENT #3: Save as high-quality PNG file
    ...
    fig.savefig(
        ...
        bbox_inches='tight', # Tight bounding box to eliminate white space
...
def run_enhanced_analysis():
    """Execute complete simulation with all improvements"""
    
    print("🔄 Enhanced 20-Day Rebalancing Monte Carlo Simulation")
    ...
    # Generate enhanced plots with PNG export
    ...
    print(f"   ✅ Final Equity Distribution X-axis limited to 1,500")
    print(f"   ✅ Increased subplot spacing (wspace=0.35, hspace=0.45)")
    print(f"   ✅ High-resolution PNG export (300 DPI)")
    print(f"\n📁 Generated files:")
    print(f"   🖼️  PNG dashboard: {png_file}")
    print(f"   📊 Excel files: {file_16x}, {file_24x}")
    print(f"   📋 Each Excel file contains 1,000 detailed simulation paths")
    
    return results_16x, results_24x, file_16x, file_24x, png_file

# Execute the enhanced analysis
if __name__ == "__main__":
    results_16x, results_24x, excel_16x, excel_24x, png_file = run_enhanced_analysis()
